...
export default function Schedules() {
  ...
  return (
    <div className="space-y-6">
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-white shadow-xl ${
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2.5">
            <CalendarDays className="w-6 h-6 text-indigo-600" /> Class Schedules
          </h1>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center rounded-2xl bg-white border border-slate-200 shadow-xs">
          ...
          <p className="text-base font-bold text-slate-700">No schedules found</p>
          ...
          {schedules.map((sch) => (
            ...
            >
              <div>
                ...
                <div className="mt-4 space-y-2 text-xs text-slate-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{sch.day}, <strong>{sch.start_time} - {sch.end_time}</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MapPin className="w-3.5 h-3.5 text-slate-400" />
                    <span>Room: <strong className="text-indigo-600">{sch.room}</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-medium">
                <span>Section: <strong className="text-slate-600">{sch.section || 'All'}</strong></span>
                <span>ID: {sch.id}</span>
              </div>
            </div>
          ))}
