    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Classrooms (7A), Labs (7B), and Seminar Halls (7C) with live bookings.</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-52 bg-white rounded-2xl border border-slate-200"></div>
          ))}
        </div>
      ) : rooms.length === 0 ? (
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className="mb-3">
                    <p className="text-[11px] font-bold text-slate-500 mb-1">Equipment:</p>
                  ...
                  {bookings.length > 0 && (
                    <div className="mt-3 p-2.5 rounded-xl bg-amber-50/70 border border-amber-200/80 space-y-1.5">
                      <p className="text-[11px] font-bold text-amber-800 flex items-center gap-1">
                        <Clock className="w-3 h-3 text-amber-600" /> Current Bookings ({bookings.length}):
                      </p>
                      {bookings.map((b) => (
                        <div key={b.booking_id} className="flex items-center justify-between text-[11px] text-slate-700 bg-white p-2 rounded-lg border border-amber-200/70 shadow-2xs">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-500">{b.date} • {b.start_time}-{b.end_time} ({b.booked_by})</p>
                          </div>
                          <button
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  <span className={`text-xs font-bold ${
                    room.status === 'available' ? 'text-emerald-600' : 'text-rose-600'
                  }`}>
                    ● {room.status}
                  </span>
                  <button
                    onClick={() => openBookModal(room)}
                    className="px-3 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 text-xs font-bold transition"
                  >
                    + Book Slot
                  </button>
                </div>
