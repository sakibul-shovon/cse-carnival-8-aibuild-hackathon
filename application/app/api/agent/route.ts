...
async function runLocalAgent(message: string, user: AuthUser) {
  ...
  if (lower.includes('register') && lower.includes('event') || lower.includes('register me')) {
    ...
    const result = await use('register_event', { event_name: eventName, student_id: user.studentId, student_name: user.fullName }, 'Register for event');
    return { message: result.already_registered ? `You’re already registered for ${String(result.event ?? '')}.` : result.ok ? `Done — you’re registered for ${String(result.event ?? '')}.` : String(result.error), trace, mode: 'local', mutated: Boolean(result.ok && !result.already_registered) };
  }
  if (lower.includes('cancel') && lower.includes('registration')) {
    ...
    const result = await use('cancel_event_registration', { event_name: eventName, student_id: user.studentId }, 'Cancel event registration');
    return { message: result.ok ? `Your registration for ${String(result.event ?? '')} has been cancelled.` : String(result.error), trace, mode: 'local', mutated: Boolean(result.ok) };
  }
  ...
  if (lower.includes('book') || lower.includes('need a room')) {
    ...
    if (roomNumber) {
      const result = await use('book_room', { room_number: roomNumber, date, ...range, booked_by: user.fullName, purpose: 'Student study session' }, 'Check availability & book room');
      return { message: result.ok ? `Booked Room ${roomNumber} for ${prettyDate(date)}, ${prettyTime(range.start_time)}–${prettyTime(range.end_time)}. Booking ID: ${String((result.booking as Record<string, unknown> | undefined)?.booking_id ?? '')}.` : String(result.error), trace, mode: 'local', mutated: Boolean(result.ok) };
    }
    ...
    const chosen = rooms[0]; const booked = await use('book_room', { room_number: chosen.room_number, date, ...range, booked_by: user.fullName, purpose: `Study session for ${capacity}` }, 'Book best matching room');
    return { message: booked.ok ? `I found and booked Room ${String(chosen.room_number)} — capacity ${String(chosen.capacity)}, with ${(chosen.equipment as string[]).join(', ')} — for ${prettyDate(date)}, ${prettyTime(range.start_time)}–${prettyTime(range.end_time)}.` : String(booked.error), trace, mode: 'local', mutated: Boolean(booked.ok) };
  }
  ...
  if (lower.includes('cancel') && lower.includes('room')) {
    ...
    const result = await use('cancel_room_booking', { room_number: roomNumber, booking_id: null, booked_by: user.fullName }, 'Cancel room booking');
  ...
  if (/where.*\b[A-Z]{3}\s?\d{4}|where.*class/i.test(message)) {
    ...
    const relevant = records.filter((item) => !course || `${String(item.title ?? '')} ${String(item.body ?? '')}`.toUpperCase().includes(course)).sort((a, b) => String(b.date).localeCompare(String(a.date)));
    if (relevant.length) return { message: `Latest notice: ${String(relevant[0].title ?? '')}. ${String(relevant[0].body ?? '')}`, trace, mode: 'local' };
  }
  ...
  if (lower.includes('next class')) {
    ...
    return { message: next ? `Your next class is ${String(next.item.course ?? '')} — ${String(next.item.title ?? '')} — on ${String(next.item.day ?? '')} at ${prettyTime(String(next.item.start_time))} in Room ${String(next.item.room ?? '')}.` : 'I couldn’t find an upcoming class.', trace, mode: 'local' };
  }
  ...
  if (day && lower.includes('class')) {
    ...
    return { message: classes.length ? `On ${day}:\n${classes.map((item) => `• ${prettyTime(String(item.start_time))} — ${String(item.course ?? '')}, ${String(item.title ?? '')}, Room ${String(item.room ?? '')}`).join('\n')}` : `You have no classes on ${day}.`, trace, mode: 'local' };
  }
  ...
  if (lower.includes('assignment') || lower.includes('due')) {
    ...
    return { message: due.length ? `Due in the next 7 days:\n${due.map((item) => `• ${String(item.course ?? '')}: ${String(item.title ?? '')} — ${prettyDate(String(item.deadline))} (${String(item.status ?? '')})`).join('\n')}` : 'Nothing is due in the next 7 days.', trace, mode: 'local' };
  }
  ...
  if (lower.includes('announcement') || lower.includes('notice')) {
    ...
    return { message: records.length ? records.map((item) => `• ${String(item.title ?? '')} — ${String(item.body ?? '')}`).join('\n') : 'No matching announcements.', trace, mode: 'local' };
  }
  ...
  if (lower.includes('lab') || lower.includes('projector') || lower.includes('fit at least')) {
    ...
    return { message: rooms.length ? `Matching rooms: ${rooms.map((room) => `${String(room.room_number ?? '')} (${String(room.capacity ?? '')} seats)`).join(', ')}.` : 'No rooms match those requirements.', trace, mode: 'local' };
  }
  ...
  if (lower.includes('free until') || lower.includes('drop into')) {
    ...
    return { message: events.length ? `You can make: ${events.map((event) => `${String(event.name ?? '')} at ${prettyTime(String(event.start_time))} in ${String(event.venue ?? '')}`).join('; ')}.` : `There aren’t any drop-in events today before ${prettyTime(end)}.`, trace, mode: 'local' };
  }

  const summary = await use('get_campus_data', { system: 'announcements' }, 'Search current campus data'); const matches = (summary.records as Record<string, unknown>[]).filter((item) => `${String(item.title ?? '')} ${String(item.body ?? '')}`.toLowerCase().includes(lower));
  return { message: matches.length ? matches.map((item) => `${String(item.title ?? '')}: ${String(item.body ?? '')}`).join('\n') : 'I can help with schedules, deadlines, announcements, room availability and bookings, or event registration. Try “When is my next class?”', trace, mode: 'local' };
}
...
export async function POST(request: Request) {
  try {
    ...
    if (env.OPENAI_API_KEY) {
      try { return NextResponse.json(await runOpenAIAgent(message, user)); } catch { /* fall through to the safe local agent */ }
    }
    return NextResponse.json(await runLocalAgent(message, user));
